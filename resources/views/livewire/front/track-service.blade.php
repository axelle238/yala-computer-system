            ...
            <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700">
                <form wire:submit.prevent="track" class="space-y-4">
                    <div>
                        ...
                        <input wire:model="search_ticket" type="text" class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl font-mono font-bold text-lg focus:ring-blue-500" placeholder="Contoh: SRV-20260124-ABCD">
                    ...
                    <div>
                        ...
                        @error('phone_verification') <span class="text-rose-500 text-xs block text-left mt-1">{{ $message }}</span> @enderror
                    </div>
                        ...
                        @foreach($steps as $key => $label)
                            <div class="flex flex-col items-center relative z-10 w-full group">
                                <div class="w-10 h-10 rounded-full flex items-center justify-center border-2 
                                    {{ $result->status === $key || (!$currentFound && $result->status !== 'cancelled') ? 'bg-blue-600 border-blue-600 text-white' : 'bg-slate-50 border-slate-200 text-slate-400' }}
                                    ...
                                    @elseif(!$currentFound && $result->status !== 'cancelled')
                                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
                                    @else
                                        {{ $loop->iteration }}
                                    @endif
                                </div>
                                <span class="text-xs font-bold uppercase mt-2 {{ $result->status === $key ? 'text-blue-600' : 'text-slate-400' }}">{{ $label }}</span>
                                
                                {{-- Connector Line --}}
                                @if(!$loop->last)
                                    <div class="absolute top-5 left-1/2 w-full h-[2px] -z-10 {{ (!$currentFound && $result->status !== 'cancelled') ? 'bg-blue-600' : 'bg-slate-200' }}"></div>
                                @endif
                            </div>
                            @if($result->status === $key) @php $currentFound = true; @endphp @endif
                        @endforeach
                    </div>
                </div>
                    ...
                    <div class="p-6 md:p-8 border-b border-slate-100 dark:border-slate-700 flex flex-col md:flex-row justify-between gap-4">
                        <div>
                            <div class="flex items-center gap-3 mb-2">
                                <span class="inline-block px-3 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 rounded-full text-xs font-bold uppercase tracking-wider">Service Ticket</span>
                        ...
                        <div class="text-right">
                             ...
                             <p class="text-lg font-bold text-slate-800 dark:text-white">
                                 {{ $result->estimated_completion ? $result->estimated_completion->format('d M Y') : 'Menunggu Diagnosa' }}
                             </p>
                        </div>
                    </div>
                        ...
                        <div>
                            ...
                            <div class="space-y-3 text-sm">
                                <div class="grid grid-cols-3">
                                    ...
                                    <span class="col-span-2 font-medium text-slate-700 dark:text-slate-300">{{ $result->device_name }}</span>
                                </div>
                                <div class="grid grid-cols-3">
                                    <span class="text-slate-500">Keluhan</span>
                                    <span class="col-span-2 font-medium text-slate-700 dark:text-slate-300">{{ $result->problem_description }}</span>
                                </div>
                                <div class="grid grid-cols-3">
                                    <span class="text-slate-500">Teknisi</span>
                                    <span class="col-span-2 font-medium text-slate-700 dark:text-slate-300">{{ $result->technician->name ?? 'Belum Ditunjuk' }}</span>
                                </div>
                            </div>
                        </div>

                        {{-- Spareparts & Cost Breakdown --}}
                        <div>
                                ...
                                <div class="space-y-2 text-sm mb-4">
                                    @foreach($result->items as $item)
                                        <div class="flex justify-between items-start">
                                            ...
                                            <span class="font-mono text-slate-600 dark:text-slate-400">Rp {{ number_format($item->price * $item->quantity, 0, ',', '.') }}</span>
                                        </div>
                                    @endforeach
                                </div>
                            @else
                            ...
                            <div class="border-t border-slate-200 dark:border-slate-700 pt-3">
                                <div class="flex justify-between items-center">
                                    ...
                                    @php 
                                        $total = $result->items->sum(fn($i) => $i->price * $i->quantity);
                                    @endphp
                                    <span class="font-black text-2xl text-emerald-600">
                                        Rp {{ number_format($total > 0 ? $total : $result->estimated_cost, 0, ',', '.') }}
                                    </span>
                                </div>
