<div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 space-y-6 animate-fade-in-up">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2">
                ...
                <div class="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-lg border border-slate-200 dark:border-slate-700 animate-slide-in-right">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input wire:model="name" type="text" class="w-full rounded-xl border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 focus:ring-violet-500 font-bold">
                        </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                @foreach($permissions as $group => $perms)
                                    <div class="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-slate-100 dark:border-slate-700">
                                        <h4 class="font-bold text-sm text-violet-600 dark:text-violet-400 uppercase mb-3">{{ $group }}</h4>
                                        <div class="space-y-2">
                                            @foreach($perms as $perm)
                                                <label class="flex items-center gap-3 cursor-pointer group">
                                                    ...
                                                    <span class="text-sm text-slate-600 dark:text-slate-300 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">{{ ucwords(str_replace('_', ' ', $perm)) }}</span>
                                                </label>
                                            @endforeach
                                        </div>
                                    </div>
                    ...
                    <div class="mt-8 flex justify-end gap-3 pt-6 border-t border-slate-100 dark:border-slate-700">
                        @if($roleId)
                            <button wire:click="delete({{ $roleId }})" class="mr-auto px-4 py-2 text-rose-500 font-bold hover:bg-rose-50 rounded-lg transition">Hapus Role</button>
                        @endif
            ...
            @else
                <div class="h-full min-h-[400px] flex flex-col items-center justify-center text-slate-400 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-3xl">
                    <svg class="w-16 h-16 mb-4 opacity-50" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
                    <p class="font-medium">Pilih role untuk mengedit atau buat baru.</p>
                </div>
            @endif
        </div>
    </div>
