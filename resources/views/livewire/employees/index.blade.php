<div class="space-y-8 animate-fade-in-up">
    ...
    @if($tampilkanForm)
        <div class="mb-10 animate-fade-in-up">
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                ...
                <form wire:submit.prevent="simpan" class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- Info Profil -->
                    ...
                    <div>
                        ...
                        <input wire:model="nama" type="text" class="w-full rounded-xl border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 focus:ring-blue-500 focus:border-blue-500 dark:text-white transition-all">
                        @error('nama') <span class="text-rose-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                    </div>
                    
                    <div>
                        <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Surel (Login)</label>
                        <input wire:model="surel" type="email" class="w-full rounded-xl border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 focus:ring-blue-500 focus:border-blue-500 dark:text-white transition-all">
                    ...
                    <div>
                        ...
                        <input wire:model="telepon" type="text" class="w-full rounded-xl border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 focus:ring-blue-500 focus:border-blue-500 dark:text-white transition-all">
                    </div>

                    <div>
                        <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Jabatan / Peran</label>
                        <select wire:model="peran" class="w-full rounded-xl border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 focus:ring-blue-500 focus:border-blue-500 dark:text-white transition-all">
                    ...
                    <div>
                        <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Gaji Pokok</label>
                        <div class="relative">
                            <span class="absolute left-3 top-2.5 text-slate-400 text-sm">Rp</span>
                            <input wire:model="gaji" type="number" class="w-full pl-10 rounded-xl border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 focus:ring-blue-500 focus:border-blue-500 dark:text-white transition-all">
                        </div>
                    </div>

                    <div>
                        <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Kata Sandi {{ $idPengguna ? '(Kosongkan jika tetap)' : '' }}</label>
                        <input wire:model="kataSandi" type="password" class="w-full rounded-xl border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 focus:ring-blue-500 focus:border-blue-500 dark:text-white transition-all">
                        @error('kataSandi') <span class="text-rose-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                    </div>

                    <div class="md:col-span-2 flex justify-end gap-3 mt-4">
                        <button type="button" wire:click="$set('tampilkanForm', false)" class="px-5 py-2.5 text-slate-600 dark:text-slate-300 font-bold hover:bg-slate-100 dark:hover:bg-slate-700 rounded-xl transition-all">Batal</button>
    ...
    <div class="bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex flex-col md:flex-row gap-4 justify-between items-center">
        ...
        <div class="text-xs text-slate-500 dark:text-slate-400 font-medium">
            Menampilkan {{ $karyawan->total() }} karyawan aktif
        </div>
    </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-lg transition-all duration-300 group relative overflow-hidden">
                ...
                <div class="p-6 flex flex-col items-center text-center">
                    ...
                    <div class="w-full grid grid-cols-2 gap-2 border-t border-slate-100 dark:border-slate-700 pt-4 mt-auto">
                        ...
                        @if($emp->id !== auth()->id())
                            <button wire:click="hapus({{ $emp->id }})" wire:confirm="Yakin ingin menghapus karyawan ini? Akses akan dicabut permanen." class="py-2 px-4 rounded-lg bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400 text-sm font-bold hover:bg-rose-100 dark:hover:bg-rose-900/40 transition-colors">
                                Hapus
                            </button>
                        @else
