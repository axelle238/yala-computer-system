<div class="relative" x-data="{ open: @entangle('terbuka') }" @click.away="open = false">
    ...
    <div x-show="open" 
        ...
        <div class="px-4 py-3 bg-slate-50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center">
            <h3 class="font-bold text-sm text-slate-800 dark:text-white">Pusat Notifikasi</h3>
            @if($adaBelumDibaca)
                <button wire:click="tandaiDibaca" class="text-xs text-indigo-600 hover:text-indigo-800 font-medium">Tandai Dibaca</button>
            @endif
        </div>

        <div class="max-h-80 overflow-y-auto custom-scrollbar">
            @forelse($notifikasi as $notif)
                <a href="{{ $notif['rute'] }}" class="block px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-700/50 border-b border-slate-50 dark:border-slate-700 last:border-0 transition-colors group">
                    <div class="flex gap-3">
                        ...
                        <div>
                            <p class="text-sm font-bold text-slate-800 dark:text-white group-hover:text-indigo-600 transition-colors">{{ $notif['judul'] }}</p>
                            <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5 line-clamp-2">{{ $notif['pesan'] }}</p>
                            <p class="text-[10px] text-slate-400 mt-1">{{ $notif['waktu']->diffForHumans() }}</p>
                        </div>
                    </div>
                </a>
            @empty
