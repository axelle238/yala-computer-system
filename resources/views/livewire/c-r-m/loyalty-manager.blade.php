<div class="space-y-8 animate-fade-in-up">
    ...
    <div class="flex justify-between items-center">
        ...
        <button wire:click="openFormPanel" class="px-6 py-3 bg-amber-500 hover:bg-amber-600 text-white font-bold rounded-xl shadow-lg transition-all flex items-center gap-2">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl border-2 border-amber-100 dark:border-amber-900/30 p-6 shadow-lg animate-fade-in-up">
            <div class="flex justify-between items-center mb-6">
                <h3 class="font-bold text-lg text-slate-800 dark:text-white flex items-center gap-2">
                    ...
                    {{ $groupIdToEdit ? 'Edit Level Membership' : 'Buat Level Baru' }}
                </h3>
                <button wire:click="closePanel" class="text-slate-400 hover:text-amber-500 transition-colors">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-4">
                    <div>
                        ...
                        <input wire:model="name" type="text" class="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-xl px-4 py-2.5 focus:ring-amber-500 focus:border-amber-500 font-bold text-slate-800 dark:text-white" placeholder="Contoh: Gold Member">
                        @error('name') <span class="text-xs text-rose-500 mt-1 block">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        <label class="block text-xs font-bold uppercase text-slate-500 mb-1">Kode Unik</label>
                        <input wire:model="code" type="text" class="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-xl px-4 py-2.5 focus:ring-amber-500 focus:border-amber-500 uppercase font-mono tracking-wider" placeholder="GOLD">
                    ...
                    <div>
                        ...
                        <select wire:model="color" class="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-xl px-4 py-2.5 text-sm focus:ring-amber-500">
                            <option value="gray">Gray (Basic)</option>
                            <option value="amber">Gold (Premium)</option>
                            <option value="slate">Platinum (Elite)</option>
                            <option value="emerald">Emerald (Exclusive)</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input wire:model="min_spend" type="number" class="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-xl focus:ring-amber-500 focus:border-amber-500 font-mono font-bold" placeholder="0">
                        </div>
                        @error('min_spend') <span class="text-xs text-rose-500 mt-1 block">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        <label class="block text-xs font-bold uppercase text-slate-500 mb-1">Diskon Otomatis (%)</label>
                        <input wire:model="discount_percent" type="number" class="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-xl px-4 py-2.5 focus:ring-amber-500 focus:border-amber-500 font-bold text-amber-600" placeholder="0">
                        @error('discount_percent') <span class="text-xs text-rose-500 mt-1 block">{{ $message }}</span> @enderror
                    </div>
                </div>
            </div>

            <div class="mt-8 flex justify-end gap-3 pt-4 border-t border-slate-100 dark:border-slate-700">
                <button wire:click="closePanel" class="px-5 py-2.5 text-slate-500 font-bold hover:bg-slate-100 dark:hover:bg-slate-700 rounded-xl transition-colors">Batal</button>
                <button wire:click="save" class="px-8 py-2.5 bg-amber-500 hover:bg-amber-600 text-white font-bold rounded-xl shadow-lg shadow-amber-500/30 transition-all transform active:scale-95">Simpan Level</button>
            </div>
        </div>
    @endif
