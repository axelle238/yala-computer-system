<div class="h-[calc(100vh-8rem)] flex flex-col lg:flex-row gap-6 overflow-hidden animate-fade-in-up relative">
    ...
    @if($statusKasir === 'closed')
        <div class="absolute inset-0 z-50 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4 rounded-3xl">
            <div class="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-2xl max-w-md w-full text-center border border-slate-200 dark:border-slate-700">
                ...
                <p class="text-slate-500 dark:text-slate-400 mb-8 text-sm">Anda harus membuka sesi kasir (Buka Shift) terlebih dahulu sebelum dapat melakukan transaksi penjualan.</p>
    ...
    <div class="flex-1 flex flex-col h-full bg-white/50 dark:bg-slate-800/50 backdrop-blur-md border border-slate-200 dark:border-slate-700 rounded-3xl shadow-sm overflow-hidden relative tech-border">
        ...
        <div class="p-4 border-b border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row gap-4 bg-white dark:bg-slate-900 sticky top-0 z-10">
            <div class="relative flex-1">
                ...
                <input wire:model.live.debounce.300ms="cari" type="text" class="block w-full pl-10 pr-3 py-2.5 border-none rounded-xl bg-slate-100 dark:bg-slate-800 text-sm focus:ring-2 focus:ring-cyan-500 transition-all placeholder-slate-500" placeholder="Scan Barcode / Cari Produk..." autofocus>
            </div>
            
            <div class="relative w-full sm:w-48">
                <input wire:model="cariRakitan" wire:keydown.enter="muatRakitan" type="text" class="block w-full px-3 py-2.5 border-none rounded-xl bg-violet-50 dark:bg-violet-900/20 text-sm focus:ring-2 focus:ring-violet-500 placeholder-violet-400 text-violet-700 dark:text-violet-300" placeholder="ID Rakitan...">
                <button wire:click="muatRakitan" class="absolute right-2 top-2 text-violet-500 hover:text-violet-700">
            ...
            <select wire:model.live="kategori" class="py-2.5 px-4 border-none rounded-xl bg-slate-100 dark:bg-slate-800 text-sm text-slate-600 dark:text-slate-300 focus:ring-2 focus:ring-cyan-500 cursor-pointer">
        ...
        <div class="flex-1 overflow-y-auto p-4 custom-scrollbar">
            <div class="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-4">
                @forelse($daftarProduk as $produk)
                    <button wire:click="tambahKeKeranjang({{ $produk->id }})" class="group relative flex flex-col bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden hover:border-cyan-500 dark:hover:border-cyan-500 hover:shadow-lg hover:shadow-cyan-500/10 transition-all text-left">
                        <div class="aspect-square bg-slate-50 dark:bg-slate-800 flex items-center justify-center p-4 relative overflow-hidden">
                            @if($produk->image_path)
                                <img src="{{ asset('storage/' . $produk->image_path) }}" class="object-contain w-full h-full mix-blend-multiply group-hover:scale-110 transition-transform duration-500">
                            @else
                            ...
                            <!-- Badge Stok -->
                            <div class="absolute top-2 right-2 px-1.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider {{ $produk->stock_quantity > 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700' }}">
                                Stok: {{ $produk->stock_quantity }}
                            </div>
                        </div>
                        <div class="p-3 flex-1 flex flex-col">
                            <h4 class="font-bold text-slate-800 dark:text-white text-xs leading-tight line-clamp-2 mb-1 group-hover:text-cyan-600 transition-colors">{{ $produk->name }}</h4>
                            <p class="text-[10px] text-slate-400 font-mono mb-2">{{ $produk->sku }}</p>
                            <div class="mt-auto flex justify-between items-end">
                                <span class="font-black font-tech text-sm text-slate-900 dark:text-slate-200">Rp {{ number_format($produk->sell_price / 1000, 0) }}rb</span>
    ...
    <div class="w-full lg:w-96 flex flex-col h-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-3xl shadow-xl overflow-hidden relative flex-shrink-0">
        ...
        <div class="p-4 bg-slate-50 dark:bg-slate-800 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center">
            <div>
                <h3 class="font-bold font-tech text-lg text-slate-900 dark:text-white">Pesanan Saat Ini</h3>
                <p class="text-[10px] text-slate-400 font-mono">{{ $nomor_referensi }}</p>
            </div>
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
            @forelse($keranjang as $index => $item)
                <div class="flex gap-3 items-start animate-slide-in-right">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <div class="flex items-center gap-2 bg-slate-100 dark:bg-slate-800 rounded-lg p-0.5">
                                <button wire:click="perbaruiJumlah({{ $index }}, {{ $item['quantity'] - 1 }})" class="w-5 h-5 flex items-center justify-center rounded bg-white dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:text-rose-500 text-xs shadow-sm">-</button>
                                ...
                                <button wire:click="perbaruiJumlah({{ $index }}, {{ $item['quantity'] + 1 }})" class="w-5 h-5 flex items-center justify-center rounded bg-white dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:text-emerald-500 text-xs shadow-sm">+</button>
                            </div>
                        </div>
                        
                        <!-- Input Nomor Seri -->
                        @if($tipe === 'out' && ($item['warranty_period'] ?? 0) > 0)
                        <div x-data="{ open: false }" class="mt-2">
                            <button @click="open = !open" class="text-[10px] text-cyan-600 dark:text-cyan-400 font-bold flex items-center gap-1 hover:underline">
                                ...
                                <span>Input Nomor Seri ({{ count($item['serial_numbers'] ?? []) }})</span>
                            </button>
                            <div x-show="open" class="mt-2 space-y-1 pl-1 border-l-2 border-slate-100 dark:border-slate-700">
                                @foreach($item['serial_numbers'] as $snIndex => $snValue)
                                    <input type="text" 
                                        wire:change="perbaruiSerial({{ $index }}, {{ $snIndex }}, $event.target.value)"
                                        value="{{ $snValue }}"
                    ...
                    <button wire:click="hapusDariKeranjang({{ $index }})" class="text-slate-300 hover:text-rose-500 transition-colors self-center">
        ...
        <div class="bg-slate-50 dark:bg-slate-800 p-4 border-t border-slate-200 dark:border-slate-700 space-y-3">
            ...
            <div class="bg-amber-50 dark:bg-amber-900/20 p-3 rounded-lg border border-amber-100 dark:border-amber-800 flex justify-between items-center">
                <div>
                    ...
                    <span class="font-bold text-amber-700 dark:text-amber-300">{{ number_format($poinPelanggan) }} Poin</span>
                </div>
                <button wire:click="tukarPoin" class="px-3 py-1.5 rounded text-xs font-bold transition-colors {{ $gunakanPoin ? 'bg-amber-500 text-white' : 'bg-white border border-amber-300 text-amber-600 hover:bg-amber-100' }}">
                    {{ $gunakanPoin ? 'BATAL' : 'TUKAR' }}
                </button>
            </div>
            @endif
            ...
            @if($this->tax > 0)
            <div class="flex justify-between text-xs text-slate-500 dark:text-slate-400">
                <span>Pajak ({{ \App\Models\Setting::get('tax_rate') }}%)</span>
            ...
            <div class="grid grid-cols-2 gap-2 mt-2">
                <button wire:click="$set('tipe', 'out')" class="py-2 text-xs font-bold rounded-lg border {{ $tipe === 'out' ? 'bg-blue-100 text-blue-700 border-blue-200' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50' }}">
                    JUAL (Keluar)
                </button>
                <button wire:click="$set('tipe', 'in')" class="py-2 text-xs font-bold rounded-lg border {{ $tipe === 'in' ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50' }}">
                    BELI (Masuk)
                </button>
            </div>

            <div class="space-y-2">
                <input wire:model="telepon_pelanggan" type="text" class="w-full text-xs px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-lg focus:ring-1 focus:ring-cyan-500" placeholder="No. HP Member (Opsional)">
                <textarea wire:model="catatan" rows="1" class="w-full text-xs px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-lg focus:ring-1 focus:ring-cyan-500" placeholder="Catatan Transaksi..."></textarea>
            </div>

            <button wire:click="simpan" class="w-full py-3 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-xl font-bold hover:shadow-lg hover:shadow-slate-900/20 transition-all flex items-center justify-center gap-2 {{ empty($keranjang) ? 'opacity-50 cursor-not-allowed' : '' }}" {{ empty($keranjang) ? 'disabled' : '' }}>
