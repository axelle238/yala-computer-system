<div class="h-[calc(100vh-8rem)] flex flex-col lg:flex-row gap-6 overflow-hidden animate-fade-in-up">
    ...
    <div class="flex-1 flex flex-col h-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl shadow-sm overflow-hidden relative">
        ...
        <div class="flex border-b border-slate-100 dark:border-slate-700">
            <button wire:click="$set('activeTab', 'diagnosis')" class="flex-1 py-4 text-sm font-bold uppercase tracking-wider border-b-2 transition-colors {{ $activeTab === 'diagnosis' ? 'border-cyan-500 text-cyan-600' : 'border-transparent text-slate-400 hover:text-slate-600' }}">Diagnosa & Catatan</button>
            <button wire:click="$set('activeTab', 'parts')" class="flex-1 py-4 text-sm font-bold uppercase tracking-wider border-b-2 transition-colors {{ $activeTab === 'parts' ? 'border-cyan-500 text-cyan-600' : 'border-transparent text-slate-400 hover:text-slate-600' }}">Sparepart & Biaya</button>
        </div>
        ...
        <div class="flex-1 overflow-y-auto p-6 custom-scrollbar relative">
            ...
            <div x-show="$wire.activeTab === 'diagnosis'" class="space-y-6">
                <div class="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-xl border border-blue-100 dark:border-blue-800/50">
                    ...
                    <textarea wire:model="technician_notes" rows="6" class="w-full bg-white dark:bg-slate-900 border-none rounded-lg focus:ring-0 text-sm" placeholder="Catat hasil diagnosa, langkah perbaikan, dan progress di sini..."></textarea>
                </div>
                            ...
                            @foreach($parts as $index => $part)
                                <tr class="bg-white dark:bg-slate-800">
                                    <td class="px-4 py-2">
                                        ...
                                        <input wire:model="parts.{{ $index }}.note" type="text" class="w-full bg-transparent border-none p-0 text-xs text-slate-400 focus:ring-0 placeholder-slate-300" placeholder="Catatan item...">
                                    </td>
                                    <td class="px-4 py-2">
