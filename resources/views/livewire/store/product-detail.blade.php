        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
            ...
            <div class="flex flex-col">
                <div class="mb-auto">
                    ...
                    <div class="flex items-center gap-4 mb-8">
                        <div class="flex items-center gap-1 text-amber-400">
                            @for($i=0; $i<5; $i++)
                                <svg class="w-5 h-5 {{ $i < round($product->ulasan->avg('rating') ?? 0) ? 'fill-current' : 'text-slate-700' }}" viewBox="0 0 20 20"><path d="M10 15l-5.878 3.09 1.123-6.545L.489 6.91l6.572-.955L10 0l2.939 5.955 6.572.955-4.756 4.635 1.123 6.545z"/></svg>
                            @endfor
                            <span class="text-slate-400 text-sm ml-2 font-mono">({{ $product->ulasan->count() }} Reviews)</span>
                        </div>
                ...
                <div class="flex gap-4 mt-8 pt-8 border-t border-white/10 sticky bottom-0 bg-slate-950/80 backdrop-blur-md p-4 -mx-4 lg:static lg:bg-transparent lg:p-0">
                    @if($product->stock_quantity > 0)
                        <button wire:click="addToCart" class="flex-1 bg-white text-black border-2 border-white hover:bg-transparent hover:text-white font-black py-4 rounded-xl transition-all shadow-[0_0_20px_rgba(255,255,255,0.3)] hover:shadow-[0_0_30px_rgba(255,255,255,0.1)] flex items-center justify-center gap-3 group active:scale-95" wire:loading.attr="disabled">
                            <span wire:loading.remove class="flex items-center gap-3">
                                ...
                                <span class="uppercase tracking-widest text-sm">Beli Sekarang</span>
                            </span>
