        ...
        @else
            <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6 animate-fade-in-up delay-100">
                @foreach($brands as $brand)
                    <a href="{{ route('toko.catalog', ['search' => $brand->name]) }}" class="group bg-white dark:bg-slate-800 p-8 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-xl hover:border-cyan-500/50 transition-all flex flex-col items-center justify-center text-center h-48 relative overflow-hidden">
