    ...
    <div class="container mx-auto px-4 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-fade-in-up delay-100">
                @foreach($daftarKeinginan as $item)
                    <div class="bg-white dark:bg-slate-800 rounded-2xl p-4 border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-lg transition-all group relative">
                        <button wire:click="hapus({{ $item->id }})" class="absolute top-3 right-3 text-slate-300 hover:text-rose-500 transition z-10">
                        ...
                        <div class="space-y-2">
                            ...
                            <button wire:click="pindahKeKeranjang({{ $item->id }}, {{ $item->product->id }})" class="w-full py-2 bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-bold rounded-lg hover:bg-cyan-600 dark:hover:bg-cyan-400 transition-colors text-sm flex items-center justify-center gap-2">
        ...
        @else
            <div class="text-center py-20 bg-white dark:bg-slate-800 rounded-3xl border border-dashed border-slate-200 dark:border-slate-700">
                ...
                <h3 class="text-xl font-bold text-slate-800 dark:text-white mb-2">Daftar Keinginan Masih Kosong</h3>
