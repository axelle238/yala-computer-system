<div class="container mx-auto px-4 py-12">
    <div class="max-w-3xl mx-auto space-y-8 animate-fade-in-up">
        ...
        <div class="text-center">
            ...
            <h1 class="text-3xl md:text-4xl font-black font-tech text-slate-900 dark:text-white mb-2">Order Diterima!</h1>
            <p class="text-slate-500 dark:text-slate-400">Terima kasih telah berbelanja di Yala Computer.</p>
        </div>

        <!-- Order Status Card -->
        <div class="bg-white dark:bg-slate-800 rounded-3xl p-6 md:p-8 shadow-xl border border-slate-100 dark:border-slate-700 overflow-hidden relative">
                ...
                @foreach($order->items as $item)
                    <div class="flex justify-between items-center">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <p class="font-bold text-slate-800 dark:text-white line-clamp-1">{{ $item->product->name }}</p>
            ...
            <div class="bg-slate-50 dark:bg-slate-900/50 rounded-2xl p-4 space-y-2 mb-8">
                ...
                @if($order->discount_amount > 0)
                    <div class="flex justify-between text-sm">
                        <span class="text-slate-500">Diskon (Poin/Voucher)</span>
                        <span class="font-mono text-emerald-600">- Rp {{ number_format($order->discount_amount + $order->voucher_discount, 0, ',', '.') }}</span>
                    </div>
                @endif
                <div class="flex justify-between items-center pt-2 border-t border-slate-200 dark:border-slate-700">
                    ...
                    <span class="text-2xl font-black font-mono text-blue-600">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</span>
                </div>
            </div>
            ...
            <div class="flex flex-col gap-3">
                @if($order->payment_status == 'unpaid' && $order->snap_token)
                    <button id="pay-button" class="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-lg shadow-blue-500/30 transition-all flex items-center justify-center gap-2">
                    ...
                    <div class="text-center text-xs text-slate-400 my-2">- ATAU -</div>
                @endif
                
                <button wire:click="sendToWhatsapp" class="w-full py-4 bg-[#25D366] hover:bg-[#20bd5a] text-white font-bold rounded-xl shadow-lg shadow-emerald-500/20 flex items-center justify-center gap-2 transition-all">
                    ...
                    Konfirmasi Pembayaran Manual (WhatsApp)
                </button>
            </div>
        </div>
