    ...
    <div class="container mx-auto px-4 lg:px-8">
        ...
        @if($featured && !$category && $articles->currentPage() == 1)
            <div class="mb-12 animate-fade-in-up delay-100">
                <a href="{{ route('store.news.show', $featured->slug) }}" class="group relative block h-[400px] md:h-[500px] rounded-3xl overflow-hidden shadow-2xl">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @forelse($articles as $article)
                ...
                <a href="{{ route('store.news.show', $article->slug) }}" class="group bg-white dark:bg-slate-800 rounded-2xl overflow-hidden shadow-sm border border-slate-200 dark:border-slate-700 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col h-full">
