<div class="max-w-5xl mx-auto space-y-8 animate-fade-in-up">
    ...
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('admin.produk.indeks') }}" class="group flex items-center gap-2 px-5 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-600 dark:text-slate-300 font-bold hover:border-blue-500 hover:text-blue-500 transition-all shadow-sm">
                ...
                <div class="space-y-6 relative z-10">
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-2">
                            <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider">Deskripsi Produk</label>
                            <button type="button" wire:click="generateAiContent" wire:loading.attr="disabled" class="text-[10px] font-bold text-white bg-gradient-to-r from-purple-500 to-indigo-600 hover:from-purple-600 hover:to-indigo-700 px-3 py-1 rounded-full shadow-md flex items-center gap-1 transition-all hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed">
                                <svg wire:loading.remove wire:target="generateAiContent" class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
                                <svg wire:loading wire:target="generateAiContent" class="w-3 h-3 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                                <span wire:loading.remove wire:target="generateAiContent">Isi Otomatis dgn AI</span>
                                <span wire:loading wire:target="generateAiContent">Berpikir...</span>
                            </button>
                        </div>
                        <textarea wire:model="description" rows="6" class="block w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-sm text-slate-700 dark:text-slate-300" placeholder="Biarkan AI menulis deskripsi menarik untuk Anda..."></textarea>
                    </div>
                </div>
            </div>
