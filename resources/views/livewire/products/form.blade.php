    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-4xl font-black font-tech text-slate-900 dark:text-white tracking-tight uppercase">
                {{ $product ? 'Edit' : 'Create' }} <span class="text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-cyan-500">Product</span>
            </h2>
                ...
                <div class="space-y-6 relative z-10">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="relative">
                                <select wire:model="category_id" class="block w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all appearance-none cursor-pointer font-medium text-slate-700 dark:text-slate-300">
                            ...
                            @error('category_id') <span class="text-xs text-rose-500 font-bold mt-1 block">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Supplier</label>
                            <div class="relative">
                                <select wire:model="supplier_id" class="block w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all appearance-none cursor-pointer font-medium text-slate-700 dark:text-slate-300">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input wire:model="buy_price" type="number" class="block w-full pl-10 pr-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all font-mono font-bold text-slate-800 dark:text-white">
                        </div>
                    </div>
                    <div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-3xl p-8 border border-slate-100 dark:border-slate-700 shadow-xl shadow-indigo-900/5 relative overflow-hidden">
                ...
                <div class="space-y-6 relative z-10">
                    <div>
                        ...
                        <input wire:model="sku" type="text" class="block w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all font-mono text-sm uppercase tracking-wide" placeholder="AUTO-GEN">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Barcode</label>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Stok Awal</label>
                            <input wire:model="stock_quantity" type="number" class="block w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all font-bold">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-rose-500 uppercase tracking-wider mb-2">Alert Minim</label>
                            <input wire:model="min_stock_alert" type="number" class="block w-full px-4 py-3 bg-rose-50/50 dark:bg-rose-900/20 border border-rose-200 dark:border-rose-800 rounded-xl focus:ring-2 focus:ring-rose-500 focus:border-transparent transition-all font-bold text-rose-600">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-3xl p-8 border border-slate-100 dark:border-slate-700 shadow-xl shadow-cyan-900/5 relative overflow-hidden">
                ...
                <div class="relative z-10">
                    <label class="block w-full aspect-square rounded-2xl border-2 border-dashed border-slate-300 dark:border-slate-600 hover:border-blue-500 dark:hover:border-blue-400 transition-all cursor-pointer bg-slate-50 dark:bg-slate-900/50 flex flex-col items-center justify-center overflow-hidden group">
                        ...
                        @else
                            <div class="flex flex-col items-center group-hover:scale-110 transition-transform duration-300">
                                ...
                                <span class="text-sm font-bold text-slate-500">Upload Foto</span>
                                <span class="text-[10px] text-slate-400 mt-1">PNG, JPG up to 2MB</span>
                            </div>
                        @endif
                    ...
                    <div wire:loading wire:target="image" class="mt-2 text-center">
                        <span class="text-xs font-bold text-blue-500 animate-pulse">Sedang mengupload...</span>
                    </div>
                </div>
            </div>

            <!-- Submit Action -->
            <button type="submit" class="w-full py-4 bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-700 hover:to-cyan-600 text-white rounded-2xl font-black text-lg shadow-xl shadow-blue-500/30 transition-all hover:-translate-y-1 hover:shadow-blue-500/50 flex items-center justify-center gap-2">
                <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
                {{ $product ? 'SIMPAN PERUBAHAN' : 'PUBLIKASIKAN PRODUK' }}
            </button>
        </div>
    </form>
</div>
